...
`tests/auth.test.mjs` boots the real `index.html` in jsdom and drives it the way a user would — filling forms, submitting them, and asserting on what renders. It covers: sign-in success and failure, all four role guards, order scoping per customer, the full basket → checkout → confirmation flow, customer and supplier sign-up (including validation and persistence across a simulated reload), the supplier approval flow end-to-end, listing a product with a newly approved account, the listing photo upload on both new and existing listings (including rejecting non-image files), and the chatbot's owner-only order access.
...
| Suppliers create and manage their own listings (FR, Must) | Supplier portal → My listings / ＋ New listing: name, description, price, unit, stock, best-before, ambient/chilled, and product photos — upload one with a new listing (live preview) or add/change it on any existing listing right from the table |
...
- **Real photos without breaking offline-first.** Hotlinking a photo CDN would break in Docker without internet, so the produce photos are openly licensed images from Wikimedia Commons, downscaled and shipped in the repo (~2 MB total). Suppliers can upload their own photo when creating a listing, or add/change one on an existing listing from the My listings table (downscaled in the browser, live in the shop immediately); listings without a photo fall back to an emoji tile.
